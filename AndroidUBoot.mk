...
$(INSTALLED_UBOOT_TARGET) : $(UBOOT_CONFIG)
	$(MAKE) -C u-boot CROSS_COMPILE=$(LOCAL_TOOLCHAIN) WDT_MONITOR_SUPPORT=$(WDT_MONITOR_EN) AP_VERSION="$(ANDROID_BUILD_DESC)" O=../$(UBOOT_OUT)
	$(MAKE) -C u-boot CROSS_COMPILE=$(LOCAL_TOOLCHAIN) WDT_MONITOR_SUPPORT=$(WDT_MONITOR_EN) AP_VERSION="$(ANDROID_BUILD_DESC)" O=../$(UBOOT_OUT) fdl2
	...
	$(MAKE) -C u-boot CROSS_COMPILE=$(LOCAL_TOOLCHAIN) WDT_MONITOR_SUPPORT=$(WDT_MONITOR_EN) AP_VERSION="$(ANDROID_BUILD_DESC)" AUTOBOOT_FLAG=true O=../$(UBOOT_OUT)
	@cp $(UBOOT_BUILT_BIN) $(PRODUCT_OUT)/${AUTOBOOT_BIN}
ifeq ($(RAM512_BINGEN_SUPPORT),true)
	$(MAKE) -C u-boot CROSS_COMPILE=$(LOCAL_TOOLCHAIN) O=../$(UBOOT_OUT) clean
	$(MAKE) -C u-boot CROSS_COMPILE=$(LOCAL_TOOLCHAIN) WDT_MONITOR_SUPPORT=$(WDT_MONITOR_EN) AP_VERSION="$(ANDROID_BUILD_DESC)" RAM_4G_SUPPORT=true ROM_8G_SUPPORT=true O=../$(UBOOT_OUT)
	$(MAKE) -C u-boot CROSS_COMPILE=$(LOCAL_TOOLCHAIN) WDT_MONITOR_SUPPORT=$(WDT_MONITOR_EN) AP_VERSION="$(ANDROID_BUILD_DESC)" RAM_4G_SUPPORT=true ROM_8G_SUPPORT=true O=../$(UBOOT_OUT) fdl2
