...
//changde
    ...
    mrctrl0 &= ~((((dram_type==DRAM_LPDDR2)||(dram_type==DRAM_LPDDR3))?0x07:0x00)<<12);
    UMCTL_REG_SET(UMCTL_MRCTRL0, mrctrl0);

    mrctrl1 = (((dram_type==DRAM_LPDDR2)||(dram_type==DRAM_LPDDR3))?mr_addr:0x00) << 8) |
              (mr_data << 0);
...
void umctl_soft_reset(BOOLEAN is_en) {
    ...
    if(is_en) {
    // Assert soft reset
    reg_value =   UMCTL_REG_GET(0x022b00c8);
    reg_value &=  ~(0x07 << 8);
    reg_value |=  0x07 << 8;
    UMCTL_REG_SET(0x022b00c8, reg_value);
    for(i = 0; i <= 1000; i++);
    } else {
    reg_value =   UMCTL_REG_GET(0x022b00c8);
    reg_value &=  ~(0x07 << 8);
...
void umctl_core_init(DRAM_DESC* dram) {
    ...
    reg_bits_set(UMCTL_MSTR,  8, 1, ((BURST_TYPE==DRAM_BT_SEQ)?0x00:0x00)|
                                    ((BURST_TYPE==DRAM_BT_INTER)?0x01:0x00));
    /*SDRAM selection for ddr2/ddr3/lpddr/lpddr2/lpddr3*/
    reg_bits_set(UMCTL_MSTR,  0, 4, ((dram_type==DRAM_DDR2)?0x00:0x00) |
                                    ((dram_type==DRAM_DDR3)?0x01:0x00) |
                                    ((dram_type==DRAM_LPDDR1)?0x02:0x00) |
                                    ((dram_type==DRAM_LPDDR2)?0x04:0x00)|
...
void umctl_sdram_timing(DRAM_DESC* dram) {
    ...
    uint8 tCKESR=((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tCKESR):0x00);
    uint8 tCKSRX=((dram_type==DRAM_DDR3)?(ddr3_timing->tCKSRX):0x00) ;
    uint8 tMOD=(dram_type==DRAM_DDR3)?(ddr3_timing->tMOD):0x00;/*DDR3 only*/
    uint8 tMRD=((dram_type==DRAM_LPDDR1)?(lpddr1_timing->tMRD):0x00)|
               ((dram_type==DRAM_DDR2)?(ddr2_timing->tMRD):0x00)|/*DDR3/2 only*/
               ((dram_type==DRAM_DDR3)?(ddr3_timing->tMRD):0x00) ;
    uint8 tRTP=((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tRTP):0x00)|
               ((dram_type==DRAM_DDR3)?(ddr3_timing->tRTP):0x00) ;
    uint8 tWTR=((dram_type==DRAM_LPDDR1)?(lpddr1_timing->tWTR):0x00)|
               ((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tWTR):0x00)|
               ((dram_type==DRAM_DDR3)?(ddr3_timing->tWTR):0x00) ;
    uint8 tRP =((dram_type==DRAM_LPDDR1)?(lpddr1_timing->tRP):0x00)|
               ((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tRP):0x00)|
               ((dram_type==DRAM_DDR3)?(ddr3_timing->tRP):0x00) ;
    uint8 tRCD=((dram_type==DRAM_LPDDR1)?(lpddr1_timing->tRCD):0x00)|
               ((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tRCD):0x00)|
               ((dram_type==DRAM_DDR3)?(ddr3_timing->tRCD):0x00) ;
    uint8 tRAS=((dram_type==DRAM_LPDDR1)?(lpddr1_timing->tRAS):0x00)|
               ((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tRAS):0x00)|
               ((dram_type==DRAM_DDR3)?(ddr3_timing->tRAS):0x00) ;
    uint8 tRRD=((dram_type==DRAM_LPDDR1)?(lpddr1_timing->tRRD):0x00)|
               ((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tRRD):0x00)|
               ((dram_type==DRAM_DDR3)?(ddr3_timing->tRRD):0x00) ;
    uint8 tRC =((dram_type==DRAM_LPDDR1)?(lpddr1_timing->tRC):0x00)|
               ((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tRC):0x00)|
               ((dram_type==DRAM_DDR3)?(ddr3_timing->tRC):0x00) ;
    uint8 tCCD=((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tCCD):0x00)|
               ((dram_type==DRAM_DDR3)?(ddr3_timing->tCCD):0x00) ; 
    uint8 tFAW=((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tFAW):0x00)|
               ((dram_type==DRAM_DDR3)?(ddr3_timing->tFAW):0x00) ;
    uint8 tRFC=((dram_type==DRAM_LPDDR1)?(lpddr1_timing->tRFC):0x00)|
               ((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tRFC):0x00)|
    ...
    uint8 tCKE=((dram_type==DRAM_LPDDR1)?(lpddr1_timing->tCKE):0x00)|
               ((dram_type==DRAM_LPDDR2)?(lpddr2_timing->tCKE):0x00)|
    ...
    uint8  tDQSCKmax=(dram_type==DRAM_LPDDR2)?(lpddr2_timing->tDQSCKmax):0x00;
    ...
    */
    ...
    reg_bits_set(UMCTL_DRAMTMG1,  8,5,((dram_type==DRAM_DDR2)?(AL+(BL>>1)+MAX(tRTP,2)-2):0x00) |
                                      ((dram_type==DRAM_DDR3)?(AL+MAX(tRTP,4)):0x00) |
                                      ((dram_type==DRAM_LPDDR1)?(BL>>1):0x00) |
                                      ((dram_type==DRAM_LPDDR2)?((BL>>1)+MAX(tRTP,2)-2):0x00) |
                                    /*((dram_type==DRAM_LPDDR2_S2)?((BL>>1)+tRTP-1):0x00) | changde
                                      ((dram_type==DRAM_LPDDR2_S4)?((BL>>1)+MAX(tRTP,2)-2):0x00) |
                                      */
                                      ((dram_type==DRAM_LPDDR3)?((BL>>1)+MAX(tRTP,4)-4):0x00) );
    ...
    (((dram_type==DRAM_DDR2)||(dram_type==DRAM_DDR3)||(dram_type==DRAM_LPDDR1))?(RL+(BL>>1)+2-WL):0x00) |
    (((dram_type==DRAM_LPDDR2)||(dram_type==DRAM_LPDDR3))?(RL+(BL>>2)+tDQSCKmax+1-WL):0x00) );
    reg_bits_set(UMCTL_DRAMTMG2,  0, 6, (WL+(BL>>1)+tWTR));

    /*tMRW, time to wait during load mode register writes.*/
    reg_bits_set(UMCTL_DRAMTMG3, 26, 6,((dram_type==DRAM_LPDDR2)?0x05:0x00) |
                                       ((dram_type==DRAM_LPDDR3)?0x0A:0x00) );
    ...
    reg_bits_set(UMCTL_DRAMTMG5, 24, 4,((dram_type==DRAM_LPDDR1)?0x01:0x00) |
                                       ((dram_type==DRAM_LPDDR2)?0x02:0x00) |
                                       ((dram_type==DRAM_LPDDR3)?0x02:0x00) |
                                       ((dram_type==DRAM_DDR2)?0x01:0x00) |
    ...
    reg_bits_set(UMCTL_DRAMTMG5, 16, 4,((dram_type==DRAM_LPDDR1)?0x00:0x00) |
                                       ((dram_type==DRAM_LPDDR2)?0x02:0x00) |
                                       ((dram_type==DRAM_LPDDR3)?0x02:0x00) |
                                       ((dram_type==DRAM_DDR2)?0x01:0x00) |
                                       ((dram_type==DRAM_DDR3)?0x05:0x00) );
    /*tCKESR,Minimum CKE low width for selfrefresh entry to exit timing in clock cycles.*/
    reg_bits_set(UMCTL_DRAMTMG5,  8, 6,((dram_type==DRAM_LPDDR1)?tRFC:0x00) |
                                       ((dram_type==DRAM_LPDDR2)?tCKESR:0x00) |
                                       ((dram_type==DRAM_LPDDR3)?tCKESR:0x00) |
                                       ((dram_type==DRAM_DDR2)?tCKE:0x00) |
                                       ((dram_type==DRAM_DDR3)?(tCKE+1):0x00) );
    ...
    reg_bits_set(UMCTL_DRAMTMG6, 24, 4,((dram_type==DRAM_LPDDR1)?0x00:0x00) |
                                       ((dram_type==DRAM_LPDDR2)?0x02:0x00) |
                                       ((dram_type==DRAM_LPDDR3)?0x02:0x00) );
    /*tCKDPDX,time before DeepPowerDownExit that CK is maintained as a valid clock before issuing DPDX.*/
    reg_bits_set(UMCTL_DRAMTMG6, 16, 4,((dram_type==DRAM_LPDDR1)?0x01:0x00) |
                                       ((dram_type==DRAM_LPDDR2)?0x02:0x00) |
                                       ((dram_type==DRAM_LPDDR3)?0x02:0x00) );
    /*tCKCSX,time before ClockStopExit that CK is maintained as a valid clock before issuing DPDX.*/
    reg_bits_set(UMCTL_DRAMTMG6,  0, 4,((dram_type==DRAM_LPDDR1)?0x01:0x00) |
                                       ((dram_type==DRAM_LPDDR2)?(tXP+0x02):0x00) |
    ...
    reg_bits_set(UMCTL_DRAMTMG7,  8, 4,((dram_type==DRAM_LPDDR1)?0x00:0x00) |
                                       ((dram_type==DRAM_LPDDR2)?0x02:0x00) |
                                       ((dram_type==DRAM_LPDDR3)?0x02:0x00) );
    /*tCKPDX,time before PowerDownExit that CK is maintained as a valid clock before issuing PDX.*/
    reg_bits_set(UMCTL_DRAMTMG7,  0, 4,((dram_type==DRAM_LPDDR1)?0x00:0x00) |
                                       ((dram_type==DRAM_LPDDR2)?0x02:0x00) |
                                       ((dram_type==DRAM_LPDDR3)?0x02:0x00) );
...
/*FPGA_TEST xiaohui in Beijing*/
    ...
    reg_bits_set(UMCTL_DFITMG0,  0,32,((dram_type==DRAM_LPDDR1)?0x02010100:0x00)|
                                      ((dram_type==DRAM_LPDDR2)?0x02050103:0x00));
...
void umctl_poweron_init(DRAM_DESC* dram) {
    ...
    reg_bits_set(UMCTL_INIT0, 16,10,((dram_type==DRAM_DDR2)?ns_to_x1024(400):0x00) | 
                                    ((dram_type==DRAM_LPDDR2)?us_to_x1024(200):0x00) |
    ...
    reg_bits_set(UMCTL_INIT0,  0,10,((dram_type==DRAM_DDR2)?us_to_x1024(200):0x00) | /*>=200us*/ 
                                    ...
                                    ((dram_type==DRAM_LPDDR2)?(ms_to_x1024(tINIT0)+ns_to_x1024(tINIT1)):0x00)|
                                    ((dram_type==DRAM_LPDDR3)?(ms_to_x1024(tINIT0)+ns_to_x1024(tINIT1)):0x00));
    ...
    reg_bits_set(UMCTL_INIT2,  8, 8,((dram_type==DRAM_LPDDR2)?us_to_x32(tINIT4):0x00));
    /*min_stable_clock_x1,time to wait after the first CKE high,tINT2.*/
    reg_bits_set(UMCTL_INIT2,  0, 4,((dram_type==DRAM_LPDDR2)?tINIT2:0x00)|
                                    ((dram_type==DRAM_LPDDR3)?tINIT2:0x00));
    ...
    */
        ...
        case DRAM_LPDDR2:
        {
            ...
            mr= ((BL==4)?0x02:0x00)|/*burst_length:2:BL4(default;3:BL8;4:BL16)*/
                ...
                ((/*0:Sequential(default);1:interleavel(allow for SDRAM only)*/
                 ((BURST_TYPE==DRAM_BT_SEQ)?0x00:0x00)|
                 ((BURST_TYPE==DRAM_BT_INTER)?0x01:0x00)
                 )<<3
                )|
    ...
    reg_bits_set(UMCTL_INIT5, 16, 8,((dram_type==DRAM_DDR3)?us_to_x32(tZQINIT):0x00) | 
                                    ((dram_type==DRAM_LPDDR2)?us_to_x32(tZQINIT):0x00)|
                                    ((dram_type==DRAM_LPDDR3)?us_to_x32(tZQINIT):0x00));
    /*max_auto_init_x1024,max duration of the auto initialization,tINIT5.*/
    reg_bits_set(UMCTL_INIT5,  0,10,((dram_type==DRAM_LPDDR2)?us_to_x1024(tINIT5):0x00)|
                                    ((dram_type==DRAM_LPDDR3)?us_to_x1024(tINIT5):0x00));
...
void phy_sdram_timing(DRAM_DESC* dram)
{
    ...
    */
    ...
    {/*dram configuration register (DCR)*/
        /*[2:0]:SDram ddr mode.*/
        reg_bits_set(PUBL_DCR, 0, 3, ((dram_type==DRAM_LPDDR1)?0x00:0x00)|
                                     ((dram_type==DRAM_DDR)?0x01:0x00)|
                                     ((dram_type==DRAM_DDR2)?0x02:0x00)|
                                     ((dram_type==DRAM_DDR3)?0x03:0x00)|
                                     ((dram_type==DRAM_LPDDR2)?0x04:0x00));
    ...
    //PTR1, to set tINT0,tINT1
    {
        ...
        uint32 tINT1 = ((dram_type==DRAM_DDR3)?0x05:0x00) |/*Unit:clk*/
                       ((dram_type==DRAM_DDR2)?ns_to_xclock(400):0x00) |
                       ((dram_type==DRAM_DDR)?0x01:0x00) |/*400ns or 1tCK*/
    ...
    //PTR2, to set tINT2,tINT3
    {
        ...
        uint32 tINT3 = (dram_type==DRAM_LPDDR2)?ns_to_xclock(1000):0x00;
        ...
        for(B1 = 0; B1 < 16; B1++)
        {
            ...
            for(B2 = 0; B2 < 16; B2++)
            {
                ...
                for(B3 = 0; B3 < 16; B3++)
                {
                    ...
                    {
                        UMCTL_REG_SET((SDRAM_BASE+0x1000+i), (0xBA55<<16)|(B3<<12)|(B2<<8)|(B1<<4)|B0);

        UMCTL_REG_SET(PUBL_DX0DQSTR, (B0<<4)|B0);
        UMCTL_REG_SET(PUBL_DX1DQSTR, (B1<<4)|B1);
        UMCTL_REG_SET(PUBL_DX2DQSTR, (B2<<4)|B2);
        UMCTL_REG_SET(PUBL_DX3DQSTR, (B3<<4)|B3);
                        return 1;
                    }
                    else
                    {
                        UMCTL_REG_SET((SDRAM_BASE+0x1000+i), (0xFA11<<16)|(B3<<12)|(B2<<8)|(B1<<4)|B0);
                    }
                    i = i + 4;
                }
            }
...
void phy_datx8_config(DRAM_DESC* dram) {
    ...
    /*[1]:*/
    reg_bits_set(PUBL_DXCCR,  1, 1, ((dram_type==DRAM_LPDDR2)?0x00:0x01) );
    /*[7:4]:*/
    reg_bits_set(PUBL_DXCCR,  4, 4, ((dram_type==DRAM_LPDDR2)?0x04:0x00)|
                                    ((dram_type==DRAM_LPDDR1)?0x04:0x00));
    /*[11:8]:*/
    reg_bits_set(PUBL_DXCCR,  8, 4, ((dram_type==DRAM_LPDDR2)?0x0C:0x00)|
                                    ((dram_type==DRAM_LPDDR1)?0x0C:0x00));
    /*[14]:*/
    reg_bits_set(PUBL_DXCCR, 14, 1, ((dram_type==DRAM_LPDDR2)?0x00:0x01) );
...
if(dram_type==DRAM_DDR3)
{
    ...
    reg_bits_set(PUBL_MR0,  3, 1, ((BURST_TYPE==DRAM_BT_SEQ)?0x00:0x00)|
                                  ((BURST_TYPE==DRAM_BT_INTER)?0x01:0x00));
...
else if(dram_type==DRAM_LPDDR1)
{
    ...
    reg_bits_set(PUBL_MR2,  5, 3, ((IO_DS==DS_FULL)?0x00:0x00)|
                                  ((IO_DS==DS_HALF)?0x01:0x00)|
                                  ((IO_DS==DS_QUARTER)?0x02:0x00)|
                                  ((IO_DS==DS_OCTANT)?0x03:0x00)|
                                  ((IO_DS==DS_THREE_QUATERS)?0x04:0x00)
...
else if(dram_type==DRAM_LPDDR2)
{
    ...
    reg_bits_set(PUBL_MR1,  0, 3, ((BL==4)?0x02:0x00)|
                                  ...
                                  ((BL==16)?0x04:0x00));
    /*BurstType:0-sequential;1-interleave*/
    reg_bits_set(PUBL_MR1,  3, 1, ((BURST_TYPE==DRAM_BT_SEQ)?0x00:0x00)|
                                  ((BURST_TYPE==DRAM_BT_INTER)?0x01:0x00));
    ...
    /*Mode register 2(MR2)*/
    reg_bits_set(PUBL_MR2,  0, 4, (((RL==3)&&(WL==1))?0x01:0x00) |
                                  (((RL==4)&&(WL==2))?0x02:0x00) |
                                  (((RL==5)&&(WL==2))?0x03:0x00) |
                                  (((RL==6)&&(WL==3))?0x04:0x00) |
                                  (((RL==7)&&(WL==4))?0x05:0x00) |
    ...
    /*Mode register 3(MR3)*/
    /*DS,driver strength configuration.Default,40ohm*/
    reg_bits_set(PUBL_MR3,  0, 4, ((IO_DS==DS_34R3)?0x01:0x00) |
                                  ((IO_DS==DS_40R)?0x02:0x00) |
                                  ((IO_DS==DS_48R)?0x03:0x00) |
                                  ((IO_DS==DS_60R)?0x04:0x00) |
                                  ((IO_DS==DS_68R6)?0x05:0x00) |
...
*/
void sdram_init(DRAM_DESC* dram) {
/*NOTE:
