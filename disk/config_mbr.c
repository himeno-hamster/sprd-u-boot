           ...
           uint32_t start, uint32_t len)
{
    ...
    printf("Configuring pentry. status=0x%x type=0x%x \n\t start_lba=%u len_lba=%u\n",
...
kb_to_lba(uint32_t len_kb, uint32_t sect_size)
{
    ...
    if (lba >= 0xffffffffULL)
        printf("Error converting kb -> lba. 32bit overflow, expect weirdness");
              ...
              uint32_t *lba)
{
    ...
    if (pnum >= PC_NUM_BOOT_RECORD_PARTS) {
        printf("Maximum number of primary partition exceeded.");
        return NULL;
    }

    if (!(item = alloc_wl(sizeof(struct pc_partition)))) {
        printf("Unable to allocate memory for partition entry.");
        return NULL;
    }
              ...
              uint32_t ext_lba, struct part_info *pnext)
{
    ...
    if (!(item = alloc_wl(sizeof(struct pc_boot_record)))) {
        printf("Unable to allocate memory for EBR.");
        return NULL;
    }
...
config_mbr(struct disk_info *dinfo)
{
    ...
    for (cnt = 0; cnt < dinfo->num_parts; ++cnt) {
        ...
        if (cnt == (PC_NUM_BOOT_RECORD_PARTS - 1)) {
            if (cnt + 1 < dinfo->num_parts) {
                ...
                if ((temp_wr = mk_pri_pentry(dinfo, NULL, cnt, &cur_lba)))
                    ...
                    printf("Cannot create primary extended partition.");
                    goto fail;
                }
            }
        }
        ...
        if (temp_wr)
            ...
            printf("Cannot create partition %d (%s).", cnt, pinfo->name);
            goto fail;
        }
    }
    ...
    for (; cnt < PC_NUM_BOOT_RECORD_PARTS; ++cnt) {
        ...
        if (!(temp_wr = mk_pri_pentry(dinfo, &blank, cnt, &cur_lba))) {
            printf("Cannot create blank partition %d.", cnt);
            goto fail;
        }
        wlist_add(&wr_list, temp_wr);
    }
...
nospace:
    printf("Not enough space to add parttion '%s'.", pinfo->name);
