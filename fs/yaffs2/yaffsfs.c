...
static yaffs_Object *yaffsfs_FindObject(yaffs_Object *relativeDirectory, const char *path,int symDepth)
{
	...
	dir = yaffsfs_FindDirectory(relativeDirectory,path,&name,symDepth);
        PRINT("yaffsfs_FindObject-0 dir %x \n", dir);

	if(dir && *name)
	{
		PRINT("yaffsfs_FindObject-0-0 dir %x \n", dir);
...
static int yaffsfs_DoStat(yaffs_Object *obj,struct yaffs_stat *buf)
{
	...
	PRINT("yaffsfs_DoStat-0 obj %x \n", obj);

	if(obj)
	{
		obj = yaffs_GetEquivalentObject(obj);
		PRINT("yaffsfs_DoStat-0-0 obj %x \n", obj);
	}
	...
	if(obj && buf)
	{
		...
		PRINT("yaffsfs_DoStat obj->variantType %x \n", obj->variantType);
		PRINT("yaffsfs_DoStat buf %x\n", buf);
	        PRINT("yaffsfs_DoStat retValadd %x\n", &retVal);
		PRINT("yaffsfs_DoStat buf->st_size %x\n", buf->st_size);
		PRINT("yaffsfs_DoStat buf->st_blksize %x\n", buf->st_blksize);
		PRINT("yaffsfs_DoStat buf->st_blocks %x\n", buf->st_blocks);
		PRINT("yaffsfs_DoStat buf->yst_atime %x\n", buf->yst_atime);
		PRINT("yaffsfs_DoStat buf->yst_ctime %x\n", buf->yst_ctime);
		PRINT("yaffsfs_DoStat buf->yst_mtime %x\n", buf->yst_mtime);
		retVal = 0;
	}
	return retVal;
}
...
static int yaffsfs_DoStatOrLStat(const char *path, struct yaffs_stat *buf,int doLStat)
{
	...
	PRINT("yaffsfs_DoStatOrLStat %x\n",obj);

	if(!doLStat && obj)
	{
		obj = yaffsfs_FollowLink(obj,0);
	}
	PRINT("yaffsfs_DoStatOrLStat-1 %x\n",obj);
	if(obj)
	{
