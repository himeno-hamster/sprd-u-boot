...
#else
#define MTDPARTS_DEFAULT "mtdparts=sprd-nand:256k(spl),512k(2ndbl),256k(params),512k(vmjaluna),10m(modem),3840k(fixnv),3840k(backupfixnv),5120k(dsp),3840k(runtimenv),10m(boot),10m(recovery),300m(system),135m(userdata),20m(cache),256k(misc),1m(boot_logo),1m(fastboot_logo),3840k(productinfo),512k(kpanic)" 
#endif
...
#define CONFIG_BOOTARGS "mem=128M init=/init "MTDPARTS_DEFAULT CONFIG_UART_CONSOLE
#else
#define CONFIG_BOOTARGS "mem=128M init=/init "MTDPARTS_DEFAULT CONFIG_UART_CONSOLE
#endif
...
//#define CONFIG_FB_LCD_ILI9806C_RGB_SPI
