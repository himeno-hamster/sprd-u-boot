...
static void recovery_sector(const char* dst_sector_path,
        ...
        unsigned int size)
{
    ensure_path_mounted(dst_sector_path);
    ...
    // ensure_path_umounted(dst_sector_path);
}
        ...
        unsigned int size)
{
    ...
    //mount yaffs
    ensure_path_mounted(sector_path);
...
TRY_BACKUP_FILE:
    ...
    //unmout yaffs
    //    ensure_path_umounted(sector_path);
...
int try_update_spl(void){
    ...
    ensure_path_mounted(tmp_mount_point);
    ...
    // ensure_path_umounted(tmp_mount_point);
...
int try_load_fixnv(void){
    ...
    ensure_path_mounted(nvitem_point);
...
EXIT:
    // ensure_path_umounted(nvitem_point);
    return ret;
}
...
static void clean_up_runtimenv(void){
    char *runtimenvpoint = "/runtimenv";
    char *runtimenvfilename = "/runtimenv/runtimenv.bin";
    char *runtimenvfilename2 = "/runtimenv/runtimenvbkup.bin";

    ensure_path_mounted(runtimenvpoint);
    cmd_yaffs_rm(runtimenvfilename);
    cmd_yaffs_rm(runtimenvfilename2);
//    ensure_path_umounted(runtimenvpoint);
}
int update_fixnv_by_id(unsigned char*addr,unsigned short id,
    ...
    unsigned int item_base = 0,length = 0;
    unsigned short item_len = 0;

    item_base = Vlx_GetFixedBvitemAddr(id, addr, (addr+FIXNV_SIZE));
    if(item_base == 0xffffffff){
        return -1;
    }
    item_len =*(unsigned short*)(item_base-2);
    if(item_len != size){
        printf("item_len = %d,len = %d,mismatch!!!\n",item_len,len);
        return -1;
    }
    memcpy(item_base+offset,data,len);
...
int update_fixnv_by_id_flush(unsigned char*addr){
    ...
    char *backupfixnvpoint = "/backupfixnv";
    char *backupfixnvfilename = "/backupfixnv/fixnv.bin";
    unsigned short write_crc=0,read_crc=0;

    length = Vlx_CalcFixnvLen(addr, addr+FIXNV_SIZE);
    *((unsigned int*)&addr[FIXNV_SIZE-8]) = length;//keep the real  fixnv file size.
    *((unsigned short*)&addr[FIXNV_SIZE-4]) = calc_checksum(addr,FIXNV_SIZE-4);
    *((unsigned short*)&addr[FIXNV_SIZE-2]) = crc16(0,(unsigned const char*)addr,FIXNV_SIZE-2);
    *(unsigned int*)&addr[FIXNV_SIZE] = 0x5a5a5a5a;
    write_crc = *((unsigned short*)&addr[FIXNV_SIZE-2]);

    printf("try save to fixnv!\n");
    recovery_sector(fixnvpoint,
            fixnvfilename,
            fixnvfilename2,
            (unsigned char *)addr,
...
//    ensure_path_umounted(fixnvpoint);
    printf("try save to backupfixnv!\n");
    recovery_sector(backupfixnvpoint,
            backupfixnvfilename,
            0,
            (unsigned char *)addr,
            FIXNV_SIZE + 4);
//    ensure_path_umounted(backupfixnvpoint);

    while(write_crc != read_crc){
        printf("Readback,makesure we have write the data to flash!!!\n");
        try_load_fixnv();
        read_crc = *((unsigned short*)&addr[FIXNV_SIZE-2]);
        printf("read_crc = %d,write_crc = %d\n",read_crc,write_crc);
    }
    clean_up_runtimenv();
    return 1;
}
